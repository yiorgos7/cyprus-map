...
let item = Math.floor(Math.random()*6)+1;
    ...
    switch (item) {
        case 1:
             word = nicosiaArr[Math.floor(Math.random()*nicosiaArr.length)];
            break
        case 2:
             word = limassolArr[Math.floor(Math.random()*limassolArr.length)];
            break
        case 3:
             word = larnacaArr[Math.floor(Math.random()*larnacaArr.length)];
            break
        case 4:
             word = famagustaArr[Math.floor(Math.random()*famagustaArr.length)];
            break 
        case 5:
             word = paphosArr[Math.floor(Math.random()*paphosArr.length)];
            break    
        case 6:
             word = kyreniaArr[Math.floor(Math.random()*kyreniaArr.length)];
            break
    }
    return word
}
...
function checkCityName (arr , svg){
    ...
    if (arr.includes(randomCity)) {
        svg.classList.add("true")
        result.innerText = "Σωστό"
    } else {
        svg.classList.add("false")
        result.innerText = "Λάθος"
         ...
         for (let i = 0; i<allArr.length; i++) {
             if (allArr[i].includes(randomCity)) {
                 allBtn[i].classList.add(true)
             } 
         }
    }
    
     // resets animation classes so they can repeated more than once
     for (let x = 0; x < allArr.length; x++) {
        allBtn[x].classList.add("reset")
    }
...
function checkCityName (arr , svg){
    ...
    if (arr.includes(randomCity)) {
        setTimeout(() => {svg.style.animation = "true  1500ms"}, 100);
        result.innerText = "Σωστό"
    } else {
        setTimeout(() => {svg.style.animation = "false  1500ms"}, 100);
        result.innerText = "Λάθος"
        //if player gets city wrong the right city turn green (gets the true class)
        for (let i = 0; i<allArr.length; i++) {
            if (allArr[i].includes(randomCity)) {
                setTimeout(() => {allBtn[i].style.animation = "true  1500ms"}, 100);
            } 
        }
    }
...
// RESETS ANIMATIONS SO THEY CAN BE PLAYED MORE THAN ONE TIME
function resetAnimations(){
        for (let x = 0; x<allArr.length; x++) {
            setTimeout(() => {allBtn[x].style.animation = "reset1 200ms"}, 1);
